# https://github.com/saharNooby/rwkv.cpp/blob/master/rwkv/convert_pytorch_to_ggml.py
...
def parse_args():
    ...
    parser.add_argument('vocab_path', help='Path to vocab file')
...
def write_state_dict(state_dict: Dict[str, torch.Tensor], tokenizer: tokenizers.Tokenizer, dest_path: str, data_type: str) -> None:
    ...
    with open(dest_path, 'wb') as out_file:
        ...
        for i in range(n_vocab):
            ...
            out_file.write(text)
            out_file.write(struct.pack("f", 0.0))
...
def main() -> None:
    ...
    tokenizer = tokenizers.Tokenizer.from_file(args.vocab_path)

    print(f'Reading {args.src_path}')

    state_dict: Dict[str, torch.Tensor] = torch.load(args.src_path, map_location='cpu')

    write_state_dict(state_dict, tokenizer, args.dest_path, args.data_type)
