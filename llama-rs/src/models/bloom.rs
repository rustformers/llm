...
impl Model for BLOOM {
    ...
    fn load(
        ...
        let ctx_size = {
            ...
            ctx_size += mulf!(n_layer, n_embd, ggml::type_sizef(ggml::TYPE_F32)); // attention_norm_b

            ctx_size += mulf!(n_layer, 3, n_embd, n_embd, ggml::type_sizef(wtype)); //query_key_value
            ctx_size += mulf!(n_layer, 3, n_embd, ggml::type_sizef(ggml::TYPE_F32)); //query_key_value_b

            ctx_size += mulf!(n_layer, n_embd, n_embd, ggml::type_sizef(wtype)); // wo
            ctx_size += mulf!(n_layer, n_embd, ggml::type_sizef(ggml::TYPE_F32)); // wo_b

            ctx_size += mulf!(n_layer, n_embd, ggml::type_sizef(ggml::TYPE_F32)); // ffn_norm
            ctx_size += mulf!(n_layer, n_embd, ggml::type_sizef(ggml::TYPE_F32)); // ffn_norm_b

            ctx_size += mulf!(n_layer, n_ff, n_embd, ggml::type_sizef(wtype)); // w1
            ctx_size += mulf!(n_layer, n_ff, ggml::type_sizef(ggml::TYPE_F32)); // w1_b

            ctx_size += mulf!(n_layer, n_ff, n_embd, ggml::type_sizef(wtype)); // w2
            ctx_size += mulf!(n_layer, n_ff, ggml::type_sizef(ggml::TYPE_F32)); // w2_b
        ...
        let model = {
            ...
            for i in 0..n_layer {
                let layer = Layer {
                    ...
                    w1_b: context.new_tensor_1d(ggml::TYPE_F32, n_ff),
                    w2: context.new_tensor_2d(wtype, n_ff, n_embd),
                    w2_b: context.new_tensor_1d(ggml::TYPE_F32, n_embd),
                };
